...
*/
import { NgModule, Component, HostListener, ElementRef, AfterViewInit, Input } from '@angular/core';
...
export class ZluxTabbingComponent implements AfterViewInit {
  ...
  setFocusActiveTab(isForward: boolean) {
    ...
    if (this.focusableArr) {
      let element: any = this.focusableArr[this.activeTab];
  ...
  checkAndAddElmAgain(selector: string, elmIndex: number) {
    ...
    this.focusableArr[elmIndex] = newElm as HTMLElement;
    return newElm;
  }
  ...
  findAllFocusable(domRef: HTMLElement) {
    ...
    if (domRef && domRef['querySelectorAll'] != null) {
      ...
      lists = lists.sort((elm1: any, elm2: any) => {
  ...
  getAllIds(focusableArr: HTMLElement[]) {
    ...
    if (this.hiddenPos && this.hiddenIds) {
      ...
      lists.forEach((selector, idx: any) => {
